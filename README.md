...
#### `WebSocket-сервер`
Полностью вся логика ВебСокет-сервера сосредоточена в 1 файле `/websocket/socketController.js`, который экспортирует функцию, принимающую объект http-сервера и объект базы данных. Поверх них уже реализовывается логика, описанная в разделе про ВебСокет-события выше.
